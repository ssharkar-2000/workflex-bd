...
export class WalletAdminService {
  ...
  async approveTopUp(id: string, adminId: string): Promise<void> {
    const credited = await this.wallet.creditTopUp(id, ['PENDING', 'HELD'], {
      reviewedBy: adminId,
      reviewedAt: new Date(),
    });
  ...
  /**
   * Turning a declared deposit down: the money it claims was never found on
   * the receiving account. Nothing was credited, so the ledger is untouched.
   */
